...
SCRIPT_VERSION="2.1" # Verziószám frissítve (nmtui fallback javítás)
...
install_network_manager() {
    ...
    echo -e "${ORANGE}🚀 NetworkManager alapcsomag telepítése...${NC}"
    if ! apt install -y network-manager; then
        echo -e "${RED}❌ KRITIKUS HIBA: A NetworkManager alapcsomag telepítése sikertelen! Ellenőrizd a hálózati kapcsolatot!${NC}"
        read -p "$(echo -e "${ORANGE}Nyomj Entert a folytatáshoz...${NC}")"
        return 1
    fi
    echo -e "${GREEN}✅ NetworkManager alapcsomag sikeresen telepítve.${NC}"

    # 2. NMTUI telepítése (opcionális TUI) - Kétféle néven próbálkozunk
    local ui_packages=("nmtui" "network-manager-tui")
    local ui_installed=false

    echo -e "${ORANGE}🖥️ NMTUI (Text UI) telepítésének kísérlete...${NC}"
    for pkg in "${ui_packages[@]}"; do
        if apt-cache show "$pkg" &>/dev/null; then
            if apt install -y "$pkg"; then
                echo -e "${GREEN}✅ '$pkg' sikeresen telepítve.${NC}"
                ui_installed=true
                break # Megvan az UI, léphetünk tovább
            fi
        fi
    done

    if ! $ui_installed; then
        echo -e "${ORANGE}⚠️ FIGYELEM: Az NMTUI vagy a 'network-manager-tui' telepítése sikertelen volt. A NetworkManager funkcionalitás attól még működik (nmcli), de az NMTUI nem érhető el.${NC}"
    fi

    # NetworkManager engedélyezése és indítása
    systemctl unmask NetworkManager || true
    systemctl enable NetworkManager
    systemctl start NetworkManager

    echo -e "${GREEN}✅ NetworkManager elindítva.${NC}"
    log_message "NetworkManager telepítve (nmtui fallback-kal kezelve)"
    read -p "$(echo -e "${ORANGE}Nyomj Entert a folytatáshoz...${NC}")"
    return 0
}
...
manage_network_manager() {
    ...
    while true; do
        ...
        local nmtui_command="nmtui"
        if ! command -v nmtui &> /dev/null; then
            if command -v network-manager-tui &> /dev/null; then
                nmtui_command="network-manager-tui" # Alternatív parancs használata, ha van
            else
                nmtui_command="" # Nincs elérhető TUI
            fi
        fi

        if [ -n "$nmtui_command" ]; then
        ...
        case $nm_choice in
            1)
                if [ -n "$nmtui_command" ]; then
                    ...
                    "$nmtui_command" # A megtalált parancs futtatása
                else
